...
import {
  UploadCloud, FileSpreadsheet, Download, CheckCircle2, AlertCircle, X, Loader2,
  Link2,
} from "lucide-react";
import { toast } from "react-toastify";
import { autoMap, buildRows,  ColRole, downloadTemplate, exportUnmatched, matchRows, PackageRow, ParsedSheet, parseSpreadsheet, toPayload, unmappedRequired } from "./handler";
import { getInterventions, getNationalPrograms } from "@/app/api/new/search";
import { bulkUploadPackages, errMsg } from "@/app/api/new/intervention-package";
import { InterventionPackage } from "@/types/new/intervention-package";
...
export function BulkUploadPackages({
  open, onClose, onComplete, packages,
}: {
  open: boolean;
  onClose: () => void;
  onComplete: () => void;
  ...
  const ingest = useCallback(async (file: File) => {
    ...
    try {
      ...
      setRows(matchRows(buildRows(sheet, m), ivs, pgs, packages));
    } catch (e: any) {
      toast.error(errMsg(e, "Could not read the spreadsheet."));
    } finally { setBusy(false); }
  }, [packages]);
 
  // re-derive rows whenever the mapping changes (live validation)
  const remap = useCallback(async (next: Record<string, ColRole>) => {
    ...
    setRows(matchRows(buildRows(parsed, next), ivs, pgs, packages));
  }, [parsed, packages]);
 
  const setRole = (key: string, role: ColRole) => {
    // a column's role is exclusive for name/reference — clear any other column holding it
  ...
  const submit = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await bulkUploadPackages(valid.map(toPayload));
      ...
      <DialogContent className="w-[95vw] sm:max-w-2xl lg:max-w-4xl max-h-[90vh] overflow-y-auto backdrop-blur-sm">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Link2 className="h-5 w-5 text-[#27aae1]" /> Bulk link to packages
          </DialogTitle>
        </DialogHeader>
        ...
        {!parsed ? (
          <div className="space-y-3">
            ...
            >
              ...
              <p className="text-sm text-slate-600">Drop a .xlsx or .csv with a package & reference column, or</p>
            ...
            {packages.length === 0 && (
              <p className="inline-flex items-center gap-1 text-xs font-medium text-amber-600">
                <AlertCircle className="h-3.5 w-3.5" /> No packages exist yet — create one before linking.
              </p>
            )}
          </div>
        ) : (
              ...
              <table className="w-full text-sm">
                <thead className="bg-slate-50 border-b border-slate-200">
                  <tr>
                    ...
                    <th className="px-3 py-2 text-left text-[11px] font-semibold uppercase tracking-wider text-slate-400">Package</th>
