...
// //   let localRefs: string[] = [], crossRefs: string[] = [];
// //   try {
// //     new Parser(trimmed).parse();
// //     localRefs = fieldRefs(trimmed); crossRefs = criterionRefs(trimmed);
// //   } catch (e) {
// //     return { ok: false, error: e instanceof FormulaError ? e.message : "parse error", localRefs: [], crossRefs: [] };
// //   }
// //   if (opts?.fieldKeys) {
...
//     if (cyclic.has(key)) { errors[key] = "circular reference"; values[key] = null; continue; }
//     const h = byKey.get(key)!;
//     const resolve = (name: string): number => {
//       if (name === key) return toNum(original?.[name]);        // own key ← raw input
//       if (name in computed) return computed[name];
//       return toNum(original?.[name]);
//     };
//     try {
//       const out = evalNode(new Parser(h.formula!).parse(), { resolve, ref });
//       const num = typeof out === "number" ? out : Number(out);
//       if (!Number.isFinite(num)) throw new FormulaError("not a finite number");
//       const rnd = h.round == null ? DEFAULT_ROUND : h.round;
//       const p = 10 ** rnd;
//       const val = Math.round(num * p) / p;
//       computed[key] = val; values[key] = val;
//     } catch (e) {
