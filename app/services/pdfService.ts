...
export const generatePDF = async (formData: FormData): Promise<Blob> => {
  ...
  if (formData.additionalInfo && formData.additionalInfo.trim()) {
    doc.setFont('helvetica', 'bold');
    doc.text('11. Additional information:', 20, y);
    doc.setFont('helvetica', 'normal');
    
    const additionalInfoLines = doc.splitTextToSize(formData.additionalInfo, 170);
    doc.text(additionalInfoLines, 20, y + 10);
    
    y += 10 + (additionalInfoLines.length * 7);
  ...
  if (formData.uploadedDocument) {
    doc.setFont('helvetica', 'bold');
    doc.text('Supporting Document Attached:', 20, y);
    doc.setFont('helvetica', 'normal');
    doc.text(`${formData.uploadedDocument.name} (${Math.round(formData.uploadedDocument.size / 1024)}KB)`, 20, y + 10);
    y += 20;
  }
