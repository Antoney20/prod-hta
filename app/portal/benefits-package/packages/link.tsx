...
import type { InterventionPackage } from "@/types/new/intervention-package";
import type { EvidenceInterventionRef } from "@/types/new/assessment";
import type { ProgramProposal } from "@/types/new/program";
import { linkOne, errMsg } from "@/app/api/new/intervention-package";
import {
...
export function LinkProposalDialog({
  open, onClose, packages, onComplete,
}: {
  open: boolean;
  onClose: () => void;
  ...
  useEffect(() => { if (!open) { setPicked(null); setSearch(""); setPackageId(""); } }, [open]);
  ...
  // single-link routes through the bulk path (the proven write). One row: link this
  // reference to the chosen package by NAME. No PATCH to the proposal endpoint.
  const attach = async () => {
    const pkg = packages.find((p) => p.id === packageId);
    if (!picked || !pkg) { toast.error("Pick a proposal and a package."); return; }
    setSubmitting(true);
    try {
      const res = await linkOne(pkg.name, picked.ref);
      if (res.attached.length) {
        toast.success(`Linked ${picked.ref} to ${pkg.name}.`);
        onComplete(); onClose();
      } else {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="w-[95vw] sm:max-w-2xl max-h-[90vh] overflow-y-auto backdrop-blur-sm">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2"><Link2 className="h-5 w-5 text-[#27aae1]" /> Link to a package</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button disabled={!picked || packageId === "" || submitting}
