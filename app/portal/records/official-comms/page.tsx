...
const OfficialCommunicationsPage: React.FC = () => {
  ...
  const fetchRecords = async (): Promise<void> => {
    try {
      ...
      const filtered = (response.results || []).filter(
        (r: Record) => r.type === 'official_communications'
      );
      setRecords(filtered);
    } catch (err) {
      setError('Failed to fetch official communications');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreateRecord = async (): Promise<void> => {
    try {
      ...
      setError('Failed to create official communication');
  ...
  const handleEditRecord = (record: Record): void => {
    ...
    setFormData({
      title: record.title,
      type: record.type || 'official_communications',
  ...
  const handleUpdateRecord = async (): Promise<void> => {
    ...
    try {
      ...
      setError('Failed to update official communication');
    ...
    <div className="lg:p-6 p-0 space-y-6">
      {loading && (
        <div className="flex justify-center items-center py-8">
          ...
          <span className="text-gray-600">Loading official communications...</span>
        </div>
      )}
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Official Communications</h1>
          <p className="text-gray-600 mt-2">
            Manage official communications, circulars, memos, and notices
          </p>
        </div>
        ...
        <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>Create Official Communication</DialogTitle>
            </DialogHeader>
      ...
      <Card>
        <CardContent className="p-4">
          <div className="relative">
            ...
            <Input
              placeholder="Search communications by title, description, or reference number..."
              className="pl-10"
              value={searchQuery}
...
export default OfficialCommunicationsPage;