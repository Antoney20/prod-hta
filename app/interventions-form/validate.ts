...
// export const RICH_FIELDS: (keyof FormData)[] = ['beneficiary','justification',  'expectedImpact', 'additionalInfo'];
...
//     .replace(/<!--[\s\S]*?-->/g, "")                                   // 1. HTML comments incl. Word conditional blocks
//     .replace(/<(style|xml|script|title|head)\b[\s\S]*?<\/\1\s*>/gi, "") // 2. tag + inner content (kills CSS/XML dumps)
//     .replace(/(?:javascript|vbscript|data):/gi, "")
//     .replace(/(?:\.\.[/\\])+/g, "")
//     .replace(/\bon\w+\s*=\s*(?:"[^"]*"|'[^']*'|[^\s>]*)/gi, "")
//     .replace(/<([^>]+)>/g, (full, inner) => (isAllowedTag(inner) ? full : "")) // 3. remaining disallowed tags (namespaced w:/o:/m: fall here)
//     .replace(/(\r?\n){3,}/g, "\n\n")
//     .trim();
// }
