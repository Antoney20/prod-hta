...
export default function NewsClient() {
  ...
  const availableCategories = useMemo(() => {
    const categoriesSet = new Set(sortedNews.map(item => item.category).filter(Boolean))
    return Array.from(categoriesSet)
  }, [sortedNews])
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2">
                ...
                <div className="p-8 flex flex-col justify-center">
                  ...
                  <div className="flex justify-between items-center">
                    <div className="text-sm text-gray-600">
                      By <span className="font-medium">{featuredNews.author}</span>
                      {featuredNews.author_role && (
                        <span className="text-gray-500">, {featuredNews.author_role}</span>
                      )}
                    </div>
                    <Link
                      href={`/news/${slugify(featuredNews.title)}`}
      ...
      <section className="py-12 bg-gray-50">
        <div className="container mx-auto px-4">
          ...
          >
            {filteredNews.map((news) => {
              const slug = slugify(news.title)
              const newsDate = new Date(news.date || news.created_at).toLocaleDateString('en-US', {
                year: 'numeric',
                month: 'long',
                day: 'numeric'
              })
              ...
              return (
                ...
                >
                  ...
                  <div className="p-5">
                    <div className="text-sm text-gray-500 mb-2">{newsDate}</div>
                    <h3 className="text-xl font-bold text-gray-900 mb-2 group-hover:text-[#27aae1] transition-colors">
                      {news.title}
                    </h3>
                    <p className="text-gray-600 mb-4 line-clamp-3">{news.excerpt }</p>
