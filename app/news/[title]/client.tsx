...
const NewsDetailClient = () => {
  ...
  const fetchNewsData = async () => {
    try {
      ...
      const newsResults = response.results || []
      setAllNews(newsResults)
    } catch (err) {
...
const newsItem = allNews.find((item: News) => {
      ...
      <main className="min-h-screen bg-white">
        ...
        <div className="flex items-center justify-center h-screen">
          <div className="text-center">
            <p className="text-red-600 text-lg mb-4">News article not found</p>
            <Link
              href="/news"
              className="px-4 py-2 bg-[#27aae1] text-white rounded-lg hover:bg-[#63C5DA] transition-colors"
            >
              Back to News
            </Link>
          </div>
  ...
  const recentNews = allNews
    .filter(item => item.id !== newsItem.id && item.published)
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      <section className="pt-32 pb-8 bg-gray-50 text-black">
        <div className="container mx-auto px-4">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex flex-wrap items-center gap-6 mb-8">
              {newsItem.author && (
                <div className="flex items-center text-gray-700">
                  <User className="h-4 w-4 mr-2" />
                  <span>
                    By <span className="font-medium">{newsItem.author}</span>
                    {newsItem.author_role && (
                      <span className="text-gray-600">, {newsItem.author_role}</span>
                    )}
                  </span>
                </div>
              )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            >
              ...
              <div className="mt-8 pt-8 border-t border-gray-200">
                ...
                <div className="flex space-x-4">
                  <button 
                    onClick={() => window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(newsItem.title)}&url=${window.location.href}`, '_blank')}
            ...
            >
              ...
              <div className="bg-gray-50 rounded-xl p-6">
                <h3 className="text-xl font-bold mb-4 pb-4 border-b border-gray-200">Categories</h3>
                <div className="space-y-2">
                  {Object.keys(categoryCounts).sort().map((category) => (
                    <Link 
                      key={category}
