...
import {
  buildScoreMap, collectServices, groupCriteria, scopeScored, serviceKey,
  evidenceForService, norm, hasNoServiceScope,
} from "../_lib/scoring";
import {
...
export default function PanelScoringDetailPage() {
  ...
  const [service, setService] = useState<string>(""); // "" = no-service scope
  ...
  // Offer a no-service scope ONLY when "" resolves to real evidence for some
  // criterion. A hollow {service:""} shell (zero fields) yields no tab.
  const hasNoService = useMemo(() => (target ? hasNoServiceScope(target) : false), [target]);
  const units = useMemo(
    () => (hasNoService ? ["", ...services] : services),
    [hasNoService, services]
  );

  // One-time landing: prefer the first service; fall back to "No service" only
  // when there are none. After that, only correct a scope that no longer exists,
  // so a manual "No service" click sticks.
  ...
  useEffect(() => {
    ...
    if (!units.includes(service)) setService(units[0]);
  ...
  // A scope is locked once the reviewer has committed ANY score for it — that's
  // the submit commit point, so a partial submit (e.g. 8/12) locks it too.
  const locked = target ? scopeScored(scoreMap, target.id, service) : false;
  ...
  const tab = (value: string, label: string) => {
    const done = scopeScored(scoreMap, target.id, value);
    const active = service === value;
    return (
      <button
        key={value || "__noservice"}
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Scope — services from the API, with No service last when applicable */}
      ...
      {locked && (
        <div className="flex items-center gap-2 rounded-md border border-emerald-200 bg-emerald-50 px-4 py-2.5 text-sm text-emerald-700">
          ...
          You&apos;ve already scored {service ? `service “${service}”` : "the no-service criteria"}. Scores are locked and can&apos;t be changed.
        </div>
      )}
