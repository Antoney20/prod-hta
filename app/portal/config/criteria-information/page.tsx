...
import { CriteriaForm, clearDraft } from "./cc/form";
...
function Modal({ children, onBackdropClick }: { children: React.ReactNode; onBackdropClick: () => void }) {
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-start justify-center overflow-y-auto pt-12 pb-12"
      onClick={(e) => { if (e.target === e.currentTarget) onBackdropClick(); }}>
      <div className="w-full max-w-3xl mx-4" onClick={(e) => e.stopPropagation()}>{children}</div>
    </div>
  );
}
...
export default function CriteriaInformationPage() {
  ...
  const attemptClose = useCallback((then?: () => void) => {
    ...
    if (formHasChanges.current()) { pendingClose.current = doClose; setGuardOpen(true); }
    else doClose();
  }, []);
  ...
  const handleGuardContinue = () => { setGuardOpen(false); pendingClose.current = null; };

  const openCreate = () => { formHasChanges.current = () => false; setEditing(null); setModalOpen(true); };
  const openEdit = (row: CriteriaInformation) => { formHasChanges.current = () => false; setEditing(row); setModalOpen(true); };
  const handleSuccess = () => { setModalOpen(false); setEditing(null); load(); };
  const handleDelete = async (row: CriteriaInformation) => { await deleteCriteriaInfo(row.id); await load(); };
    ...
    <div className="min-h-screen bg-linear-to-b py-12 from-slate-50 to-white">
      ...
      <div className="bg-white border-b border-slate-200">
        <div className="container mx-auto px-6 py-8 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-slate-900 font-serif">Proposal Criteria Information</h1>
            <p className="text-sm text-slate-500 mt-1">Interventions &amp; national programs — from the word docs.</p>
          </div>
      ...
      {modalOpen && (
        <Modal onBackdropClick={() => attemptClose()}>
          <div className="bg-white rounded-lg shadow-lg">
            <CriteriaForm initial={editing} onSuccess={handleSuccess} onCancel={() => attemptClose()} hasChangesRef={formHasChanges} />
          </div>
        </Modal>
      )}
      ...
      <AlertDialog open={guardOpen} onOpenChange={setGuardOpen}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleGuardDiscard} className="bg-red-600 hover:bg-red-700 focus:ring-red-600">Discard changes</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
