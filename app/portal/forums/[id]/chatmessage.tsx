...
const ChatMessage: React.FC<EnhancedChatMessageProps> = ({ 
  ...
  const showParentPreview = message.parent_message && isReply && !isOwn;
    ...
    <div className={`group relative py-3 hover:bg-gray-50/50 transition-colors duration-150 ${isReply ? 'ml-12 pl-4 border-l-2 border-gray-200' : ''}`}>
      <div className={`flex items-start space-x-3 ${isOwn ? 'flex-row-reverse space-x-reverse' : ''}`}>
        ...
        <div className={`flex-1 min-w-0 ${isOwn ? 'text-right' : ''}`}>
          ...
          {showParentPreview && (
            <div className="mb-2 rounded-2xl overflow-hidden border border-gray-200 max-w-full">
              <div className="bg-green-200 px-4 py-2 border-b border-green-300">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs font-medium text-green-800">
                    {getUserDisplayName(message.parent_message!.user)}
                  </span>
                  <span className="text-xs text-green-600">
                    {formatMessageTime(message.parent_message!.created_at)}
                  </span>
                </div>
                <p className="text-xs text-green-900 line-clamp-1 leading-relaxed">
