...
const AttendanceRegistersPage: React.FC = () => {
  ...
  const fetchRecords = async (): Promise<void> => {
    try {
      ...
      const filtered = (response.results || []).filter(
        (r: Record) => r.type === 'attendance_registers'
      );
      setRecords(filtered);
    } catch (err) {
      setError('Failed to fetch attendance registers');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreateRecord = async (): Promise<void> => {
    try {
      ...
      setError('Failed to create attendance register');
  ...
  const handleEditRecord = (record: Record): void => {
    ...
    setFormData({
      title: record.title,
      type: record.type || 'attendance_registers',
  ...
  const handleUpdateRecord = async (): Promise<void> => {
    ...
    try {
      ...
      setError('Failed to update attendance register');
    ...
    <div className="lg:p-6 p-0 space-y-6">
      {loading && (
        <div className="flex justify-center items-center py-8">
          ...
          <span className="text-gray-600">Loading attendance registers...</span>
        </div>
      )}
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 mt-2">
            Manage meeting attendance, staff attendance, and attendance records
          </p>
        </div>
        ...
        <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>Create Attendance Register</DialogTitle>
            </DialogHeader>
            <div className="space-y-4">
              <div>
                ...
                <Input
                  ...
                  placeholder="e.g., Board Meeting Attendance - Jan 2025"
                  required
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label htmlFor="documents">Attendance Document</Label>
                  <Input
                    id="documents"
                    type="file"
      ...
      <Card>
        <CardContent className="p-4">
          <div className="relative">
            ...
            <Input
              placeholder="Search registers by title, description, or reference number..."
              className="pl-10"
              value={searchQuery}
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Edit Attendance Register</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div>
...
export default AttendanceRegistersPage;